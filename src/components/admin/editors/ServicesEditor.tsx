...
export function ServicesEditor({ initialData }: Props) {
  ...
  const [items, setItems] = useState<ServiceItem[]>(
    (initialData.items as ServiceItem[]) || []
  );

  function addItem() {
    setItems([
      ...items,
      { icon: "Code2", number: "", title: "", description: "", color: "#00D28C" },
    ]);
  }
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await updateSectionContent("services", {
        label,
        title,
        subtitle,
        items,
  ...
  return (
    <SectionEditorLayout
      title="Servicos"
      description="Secao de servicos oferecidos"
      onSave={handleSave}
      saving={saving}
    >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <Label className="text-sm font-semibold">Itens de Servico</Label>
          <Button
            type="button"
            variant="outline"
            size="sm"
